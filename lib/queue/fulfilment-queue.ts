...
/**
 * Enqueues placing the AliExpress order(s) for a just-paid Order.
 * jobId is the order id, not a random id -- a second enqueue for the same
 * order (a retried webhook, a re-run job) is a no-op at the queue level
 * rather than a duplicate job, on top of place-supplier-order.ts's own
 * status guard.
 */
export async function enqueuePlaceSupplierOrder(orderId: string): Promise<void> {
